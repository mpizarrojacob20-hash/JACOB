...
// WhatsApp bloquea mensajes de texto libre a alguien que no te ha escrito en las
// últimas 24 horas (error 21610/63016) — para mensajes que Jacob inicia (bienvenida,
// check-in a clientes inactivos) hay que usar una plantilla pre-aprobada por Meta en
// vez de Body.
export async function sendWhatsAppTemplate(accountSid, authToken, to, from, contentSid, contentVariables = {}) {
  const auth = Buffer.from(`${accountSid}:${authToken}`).toString("base64");
  const params = new URLSearchParams({ To: to, From: from, ContentSid: contentSid });
  ...
  if (!response.ok) {
    const errText = await response.text();
    throw new Error(`Twilio send template ${response.status}: ${errText}`);
  }
}
