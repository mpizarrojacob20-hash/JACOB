// api/jacob-agent.js — Endpoint conversacional de Jacob, con memoria en Notion
// (CLIENTES + ACCIONES). Sigue el mismo patrón de fetch crudo que notion.js/analyze.js
// — sin SDK, sin dependencias.
...
async function findOrCreateClient(token, clientesDb, telefono) {
  ...
  if (query.results.length > 0) {
    return { page: query.results[0], isNew: false };
  }

  const created = await notionFetch(token, "pages", {
    method: "POST",
    body: JSON.stringify({
      parent: { database_id: clientesDb },
      properties: {
        telefono: { title: [{ text: { content: telefono } }] },
        ultima_interaccion: { date: { start: todayISO() } },
      },
    }),
  });

  return { page: created, isNew: true };
}
...
function buildContextBlock(clientProfile, pendingActions, isNew) {
  if (isNew) {
...
export default async function handler(req, res) {
  ...
  const { ANTHROPIC_API_KEY, NOTION_TOKEN, NOTION_CLIENTES_DB, NOTION_ACCIONES_DB } = process.env;
  ...
  if (!NOTION_TOKEN || !NOTION_CLIENTES_DB || !NOTION_ACCIONES_DB) {
    return res.status(500).json({ error: "Notion no configurado (NOTION_TOKEN / NOTION_CLIENTES_DB / NOTION_ACCIONES_DB)" });
  }
  ...
  try {
    const { page: clientPage, isNew } = await findOrCreateClient(NOTION_TOKEN, NOTION_CLIENTES_DB, telefono);
    const pendingActions = isNew ? [] : await getPendingActions(NOTION_TOKEN, NOTION_ACCIONES_DB, clientPage.id);
    const contextBlock = buildContextBlock(clientPage, pendingActions, isNew);
